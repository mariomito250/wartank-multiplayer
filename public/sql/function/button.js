...
function napthecao() {
    giaotiep('Trò chơi miễn phí ! Không nạp thẻ.');
    return false;
    var am = '';
...
vatpham(46,-1);
   ...
   chat(' '+userName+' tham gia vòng quay may mắn nhận được '+shopvatpham(41).ten+'  ','CTG',0);
        ...
        function info_wild(id) {
            var ac = '';
            for (i=0;i<data_pokemon.length;i++) {
            if(data_pokemon[i].id == id)    {
ac+= '<img src="/xml/nhanvat.php?nhanvat='+data_pokemon[i].img+'&nut=1"> ';
ac+='<br> '+t('Giáp')+' : '+data_pokemon[i].giap+' ';
ac+='<br> '+t('HP')+' : '+data_pokemon[i].hp+' ';
ac+='<br> '+t('MP')+' : '+data_pokemon[i].mp+' ';
ac+='<br> '+t('Tốc độ')+' : '+data_pokemon[i].tocdo+' ';
ac+='<br> '+t('Giáp')+' : '+data_pokemon[i].giap+' ';
ac+='<br> '+t('Phép')+' : '+data_pokemon[i].phep+' ';
ac+='<br> '+t('Chí mạng')+' : '+data_pokemon[i].chimang+' ';
ac+='<br> '+t('Xuyên giáp')+' : '+data_pokemon[i].xuyengiap+' ';

}
}
                   box(t('Poke DEX'),''+t('Thông tin')+':info_wild(1);'+t('Quay lại')+':wild(1)',ac); 
    ...
    var ac='';
     ...
     if(id==1) {
         ...
         data_pokemon.sort(function(a, b){return  a.id- b.id  });
 
   for(k=0;k< data_pokemon.length;k++) {
       
           
ac+='<div class="border" onclick="info_wild('+data_pokemon[k].id+')"> <div  class="showitem" style="border: 2px solid;"><img src="/xml/nhanvat.php?nhanvat='+data_pokemon[k].img+'&nut=1"></div>'+data_pokemon[k].ten+' </div> ';
     ...
     if(id==2) {
        ac+=' <div class="border row"> <input id="idpkm" class="form-control" type="text" value="" onkeydown="if(event.keyCode == 13) timkiempkm();" style="width: 70%;" placeholder="ID POKEMON"> <div class="canvas_menu" onclick="timkiempkm()">'+t('TÌM')+'</div> </div>';
             }       
     ...
     ac+='<div id="ketqua"></div>';
       ...
       box(t('Bạn cần tra cứu thông tin pokemon'),''+t('Theo ID')+':wild(1);'+t('Tìm kiếm')+':wild(2)',ac); 
     
    }   
    
    function timkiempkm() {
        $('#ketqua').html('Không tìm thấy pokemon nào cả.');
        for(k=0;k< data_pokemon.length;k++) {
       
        if(data_pokemon[k].id == $('#idpkm').val())    {
 $('#ketqua').html('<div class="border" onclick="info_wild('+data_pokemon[k].id+')"> <div  class="showitem" style="border: 2px solid;"><img src="/xml/nhanvat.php?nhanvat='+data_pokemon[k].img+'&nut=1"></div>'+data_pokemon[k].ten+' </div> ');
}
   }
    }
...
function bxh(id) {
   ...
   for(k=0;k< mmo.length;k++) {
       if(k<20) {
           if(+mmo[k].id >=100) {
ac+='<div class="border" onclick="mmouser_info('+mmo[k].id+')"> <div  class="showitem" style="border: 2px solid;"><img src="/xml/nhanvat.php?nhanvat='+mmo[k].skin+'&nut=1"></div>'+mmo[k].username+' - '+tron(mmo[k].xu)+' XU </div> ';
}
}
   }
   ...
   for(k=0;k< mmo.length;k++) {
       if(k<20) {
           if(+mmo[k].id >=100) {
ac+='<div class="border" onclick="mmouser_info('+mmo[k].id+')"> <div  class="showitem" style="border: 2px solid;"><img src="/xml/nhanvat.php?nhanvat='+mmo[k].skin+'&nut=1"></div>'+mmo[k].username+' - '+tron(mmo[k].ruby)+' RUBY </div> ';
}
}
   }
   ...
   for(k=0;k< mmo.length;k++) {
       if(k<20) {
           if(+mmo[k].id >=100) {
ac+='<div class="border" onclick="mmouser_info('+mmo[k].id+')"> <div  class="showitem" style="border: 2px solid;"><img src="/xml/nhanvat.php?nhanvat='+mmo[k].skin+'&nut=1"></div>'+mmo[k].username+' - lv.'+tron(mmo[k].level)+' + '+tron(mmo[k].exp)+'/'+tron(mmo[k].expmax)+'  </div> ';
}
}
   }
...
function game_chorse(idx) {
    ...
    if(my.click.id !==0 && getnpc(my.click.id) !== null && my.click.type =="pokemon" ) {
           ...
           if(i <=-1 && my.danhlan[i].id != my.click.id && getnpc( my.danhlan[i].id) !== null) {
            ...
            if(getusers(my.click.id).chiso.hp >=1) {
                pvp(my.click.id);
...
function moipk(id,cuoc) {
    ...
    if(my.pk.timemoi > date()) {
      ...
      my.pk.timemoi = date()+15000;
            socket.emit("moipvp",{nguoimoi : userID ,  nguoinhan : id , cuoc : cuoc  });		
...
function pvp_dongy(id,cuoc) {
 ...
 socket.emit("dongypvp",{nguoimoi : id ,  nguoinhan : userID , cuoc : cuoc  });		
 c_giaotiep();
 ketnoi();
...
my.kethu.sort(function(a, b){return b.tg -  a.tg  });
        ...
        for(var i=0;i<my.kethu.length;i++) {
            ...
            ac+='<div class="border" onclick="mmouser_info('+my.kethu[i].id+',1)"> <div  class="showitem" style="border: 2px solid;"><img src="/xml/nhanvat.php?nhanvat='+my.kethu[i].skin+'&nut=1"></div><font color="'+clor_x+'">'+my.kethu[i].name+' ('+my.kethu[i].thoigian+')</font></div> ';
        }
    }
    if(id==8) {
     ...
     ac+='<div class="border" onclick="mmouser_info('+my.banbe[i].id+',1)"> <div  class="showitem" style="border: 2px solid;"><img src="/xml/nhanvat.php?nhanvat='+my.banbe[i].skin+'&nut=1"></div><font color="'+clor_x+'">'+my.banbe[i].name+'</font></div> ';
        }
    }
    if(id==7) {
...
ac+='<center><button class="btn btn-primary" onclick="ban1('+idvp+')" id="nutban">'+t('Bán')+'</button></center>';
    ...
    if(id==1) {
        ...
        ac+='<br>- '+t('Tiến độ')+' : <b>'+t('Hoàn thành')+'</b> ';
        } else {
         ac+='<br>- '+t('Tiến độ')+' : <b>'+my.users.nhiemvu.song+'/'+my.users.nhiemvu.can+'</b> ';
           
        }
    ac+='<br>- '+t('Nơi nhận')+' : <b>'+nhiemvu(my.users.nhiemvu.id).npcname+'</b> ';
    ac+='<br>- '+t('Thưởng')+' : ';
...
if(nhiemvu(my.users.nhiemvu.id).ducnghia.vatpham.split(",")[i].split(":")[0] >0) {
   ...
   if(my.users.nhiemvu.code ==0) {
    ...
    ac+='<br>-  '+t('Tới gặp')+' : <b>'+nhiemvu(my.users.nhiemvu.id).npcname+'</b> ';
...
ac+='<div class="border"> <div  class="showitem" style="border: 2px solid;"></div></div>';
    ...
    if(id==4) {
      ac+='<div class=" menu list" onclick="game_hanhtrang(9)"><font color="red">'+t('Hưỡng dẫn chơi')+'</font></div>';
        ...
        ac+='<div class=" menu list" onclick="edit_info()">'+t('Đổi mật khẩu')+'</div>';
    ...
    ac+='<div class=" menu list" onclick="ngonngu()">'+t('Ngôn ngữ')+'</div>';
        ...
        ac+=' <center>'+t('Tài khoản')+' : '+my.users.taikhoan+' <br> '+t('Nhân vật')+' : '+my.users.username+' <br> '+t('Máy chủ')+' : Mặt Trời</center>';
    }
    ...
    box('<img src="/xml/nhanvat.php?nhanvat='+my.users.skin+'&nut=1"> '+my.users.username+' #'+my.users.id+' <br> '+t('Cấp độ')+' : '+my.users.level+'<span class="trai">('+tron(my.users.exp)+'/'+tron(my.users.expmax)+')</span>',''+t('Nhiệm vụ')+':game_hanhtrang(1);'+t('Túi')+':game_hanhtrang(2);Pokemon:game_hanhtrang(3);'+t('Cài đặt')+':game_hanhtrang(4)',ac); 
...
if(+ruongtrangbi(idtb).chiso.mp >=1) 
 ...
 if(+ruongtrangbi(idtb).chiso.tocdo >=1) 
        ...
        $('#trangbi2_'+idtb).html(''+ruongtrangbi(idtb).chiso.ten+' + '+ruongtrangbi(idtb).chiso.nangcap+' <br> '+ty+'');  
...
if(+ruongtrangbi(idtb).chiso.mp >=1) 
 ...
 if(+ruongtrangbi(idtb).chiso.tocdo >=1) 
    ...
    if(+idpkm == +my.users.pokemon) {
        ...
        $('#trangbi_'+idtb).html(''+ruongtrangbi(idtb).chiso.ten+' + '+ruongtrangbi(idtb).chiso.nangcap+' <br> '+ty+'');  
...
kkykrtyrty+='<div class="border" onclick="game_docsach('+data_kinang[k].id+')"> <div  class="showitem" style="border: 2px solid;"><img src="/sql/eff/skill/'+data_kinang[k].id+'.png" ><div class="count count-vp-1">lv.'+data_kinang[k].level+'</div></div>'+data_kinang[k].ten+'</div>';
}
...
kkykrtyrty+='<div class="border" onclick="game_docskin(\''+data_shopskin[k].ten+'\')"> <div  class="showitem" style="border: 2px solid;"><img src="/xml/nhanvat.php?nhanvat='+data_shopskin[k].ten+'.png&nut=1" ><div class="count count-vp-1">VV</div></div> '+t('Giá')+' : '+data_shopskin[k].gia+' '+data_shopskin[k].loai+' <div id="skin_'+data_shopskin[k].ten+'" class=" menu list" style="display:none;"></div></div>';
		}
			}
    }
    if(id==2) {
         for(var k=0;k<my.thoitrang.length;k++) {
kkykrtyrty+='<div class="border" onclick="game_docskin(\''+my.thoitrang[k].ten+'\')"> <div  class="showitem" style="border: 2px solid;"><img src="/xml/nhanvat.php?nhanvat='+my.thoitrang[k].ten+'.png&nut=1"><div class="count count-vp-1">VV</div></div> '+t('Chạm để xem')+' <div id="skin_'+my.thoitrang[k].ten+'" class=" menu list" style="display:none;"></div></div>';
		}
    }
...
giaotiep('Giá tiền không hợp lệ..');
   return false;
    ...
    c_box();
    socket.emit("ban_1",{idvp : id ,  nguoiban : userID , nguoimua : +getname(us).id , xu : x , ruby : r , soluong : sl  });		
...
function dongymua(idvp,soluong,x,r,nguoiban) {
    ...
    giaotiep(t('Đang mua vật phẩm... xin chờ ....'));
        socket.emit("ban_2",{idvp : idvp ,  nguoiban : nguoiban , nguoimua : userID , xu : x , ruby : r , soluong : soluong  });		
...
giaotiep('Giá tiền không hợp lệ..');
   return false;
    ...
    c_box();
    socket.emit("pokemon_1",{idpkm : id ,  nguoiban : userID , nguoimua : +getname(us).id , xu : x , ruby : r , info : mypokemon(id)  });		
...
function dongymua2(idvp,x,r,nguoiban) {
    ...
    giaotiep(t('Đang mua vật phẩm... xin chờ ....'));
        socket.emit("pokemon_2",{idpkm : idvp ,  nguoiban : nguoiban , nguoimua : userID , xu : x , ruby : r   });		
...
function game_pokemon(id, num)
{
    ...
    if (num == 4)
    {
        if (+mypokemon(id).chiso.binhnuoc >= 1)
        {
            ...
            ac += '<div class="border" onclick="game_doctrangbi2(' + id + ',' + idtb + ')"> <div  class="showitem" style="border: 2px solid;"><img src="/sql/item/' + ruongtrangbi(idtb).chiso.img + '.png" ><div class="count count-vp-1">+' + ruongtrangbi(idtb).chiso.nangcap + ' </div></div>' + ruongtrangbi(idtb).chiso.ten + '</div><div id="trangbi2_' + idtb + '" class=" menu list" style="display:none;"></div>';
        }
        else
        {
            ac += '<div class="border"> <div  class="showitem" style="border: 2px solid;"></div></div>';
    ...
    if (num == 1)
    {
        ...
        ac += t('1 trí tuệ : tăng 4 hp,0.01 xuyên giáp. <br>');
        ac += t('1 thể lực : tăng 2 hp,3 mp. <br>');
        ac += t('1 khéo léo : 3 mp,3 tốc độ <br>');
        ac += t('1 thân pháp : 2 giáp, 2 sát thương phép <br>');

    }

    box('<img src="/xml/nhanvat.php?nhanvat=' + mypokemon(id).chiso.img + '&nut=1"> ' + mypokemon(id).chiso.ten + ' #' + mypokemon(id).id + ' <br> '+t('Cấp độ')+' : ' + mypokemon(id).chiso.level + '', ''+t('Chỉ số')+':game_pokemon(' + id + ',1);'+t('Kĩ năng')+':game_pokemon(' + id + ',2);'+t('Trang bị')+':game_pokemon(' + id + ',4);'+t('Túi')+':game_pokemon(' + id + ',5);'+t('Tính năng')+':game_pokemon(' + id + ',3)', ac);
...
if(id==3) {
    ...
    box('<img src="/xml/nhanvat.php?nhanvat='+my.users.skin+'&nut=1"> '+my.users.username+' #'+my.users.id+' <br> '+t('Cấp độ')+' : '+my.users.level+'<span class="trai">('+tron(my.users.exp)+'/'+tron(my.users.expmax)+')</span>',''+t('Chát chung')+':doichat(0);Map:doichat(1);'+t('Riêng')+':doichat(3)',ac); 
...
function game_ib(id,type) {
    ...
    box('<img src="/xml/nhanvat.php?nhanvat='+my.users.skin+'&nut=1"> '+my.users.username+' #'+my.users.id+' <br> '+t('Cấp độ')+' : '+my.users.level+'<span class="trai">('+tron(my.users.exp)+'/'+tron(my.users.expmax)+')</span>', 'Inbox:game_ib(' + id + ')', ac);
...
function mmouser_info(id,tc) {
   ...
   ac+='<div id="zxczxcse" style="display:none;">'+t('Mức cược')+' : <input id="cuoc" class="form-control" type="number" value="0" style="width: 50%;">';
    ac+='<br><center><button class="btn btn-primary" onclick="gamemoipvp('+id+')" id="nutban">'+t('Thách đấu')+'</button></center></div>';
       ...
       box('<img src="/xml/nhanvat.php?nhanvat='+getusers(id).skin+'&nut=1"> '+getusers(id).username+' #'+getusers(id).id+'',''+t('Người chơi')+':mmouser_info('+id+')',ac); 
...
if(id==2) {
    for(var k=0;k<my.pokemon.length;k++) {
             if(+mypokemon(my.pokemon[k].id).id == +my.users.pokemon) 
              ...
              if(mypokemon(my.pokemon[k].id).kho ==1  ) {
		ac+='<div class="border" onclick="laypokemon('+mypokemon(my.pokemon[k].id).id+')"> <div  class="showitem" style="border: 2px solid;"><img src="/xml/nhanvat.php?nhanvat='+mypokemon(my.pokemon[k].id).chiso.img+'&nut=1" ><div class="count count-vp-1">lv.'+mypokemon(my.pokemon[k].id).chiso.level+'</div></div>'+tenpkm+'  HP : '+tron(mypokemon(my.pokemon[k].id).chiso.hp)+'/'+tron(mypokemon(my.pokemon[k].id).chiso.hpfull)+' </div>';
			}
         }
}
if(id==3) {
