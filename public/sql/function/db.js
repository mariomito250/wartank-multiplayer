...
function lichsu(text) {
  ...
  for(var k=0;k<my.lichsu.length;k++) {
				if( my.lichsu[k].xoa < date() ) {
				my.lichsu.splice(k, 1);
...
socket.emit("chatthegioi",{msg : msg ,  id : id , type : type  });		
...
if(userID >=0 && my.users.skin ) {
    if(my.users.nhiemvu.id === undefined) {
           my.users.nhiemvu.id = 1;
			...
			socket.emit("Move", ""+userName+"^"+userID+"^" + mapID +"^"  + userEvent.mapPosition.X +"^"  + userEvent.mapPosition.Y +"^"  + userEvent.direction +"^"+chiendau+"^"+userName+"^"+my.users.skin+"^"+clan_icon+"^"+clan_viettat+"^"+tagAlong+".png^"+setexp+"^"+setxu+"^"+camxuc+"^"+server+"^"+JSON.stringify(my.chiso)+"^"+my.users.co+"^0");
      socket.emit('ping','abc');
...
giaotiep(t(getnpc(scriptData).giaotiep));
 ...
 csmmdfsdf+= '<b onclick="'+getnpc(scriptData).button[i].onclick+'" class="viptxt nutchat">'+t(getnpc(scriptData).button[i].name)+'</b>';
	 }
...
if(+my.trangbi[k].chiso.chimang >=1) {
		 my.trangbi[k].chiso.chimang = +my.trangbi[k].chiso.chimanggoc + +my.trangbi[k].chiso.chimanggoc/100*+my.trangbi[k].tile + +my.trangbi[k].chiso.nangcap *+my.trangbi[k].tile;
}
if(+my.trangbi[k].chiso.hp >=1) {
		 my.trangbi[k].chiso.hp = +my.trangbi[k].chiso.hpgoc + +my.trangbi[k].chiso.hpgoc/100*+my.trangbi[k].tile + +my.trangbi[k].chiso.nangcap *+my.trangbi[k].tile;
}
if(+my.trangbi[k].chiso.phep >=1) {
		 my.trangbi[k].chiso.phep = +my.trangbi[k].chiso.phepgoc + +my.trangbi[k].chiso.phepgoc/100*+my.trangbi[k].tile + +my.trangbi[k].chiso.nangcap *+my.trangbi[k].tile;
}
if(+my.trangbi[k].chiso.vatli >=1) {
		 my.trangbi[k].chiso.vatli = +my.trangbi[k].chiso.vatligoc + +my.trangbi[k].chiso.vatligoc/100*+my.trangbi[k].tile + +my.trangbi[k].chiso.nangcap *+my.trangbi[k].tile;
}
if(+my.trangbi[k].chiso.xuyengiap >=1) {
		 my.trangbi[k].chiso.xuyengiap = +my.trangbi[k].chiso.xuyengiapgoc + +my.trangbi[k].chiso.xuyengiapgoc/100*+my.trangbi[k].tile + +my.trangbi[k].chiso.nangcap *+my.trangbi[k].tile;
}
if(+my.trangbi[k].chiso.mp >=1) {
		 my.trangbi[k].chiso.mp = +my.trangbi[k].chiso.mpgoc + +my.trangbi[k].chiso.mpgoc/100*+my.trangbi[k].tile + +my.trangbi[k].chiso.nangcap *+my.trangbi[k].tile;
}
if(+my.trangbi[k].chiso.giap >=1) {

		 my.trangbi[k].chiso.giap = +my.trangbi[k].chiso.giapgoc + +my.trangbi[k].chiso.giapgoc/100*+my.trangbi[k].tile + +my.trangbi[k].chiso.nangcap *+my.trangbi[k].tile;
}

if(+my.trangbi[k].chiso.tocdo >=1) {

		 my.trangbi[k].chiso.tocdo = +my.trangbi[k].chiso.tocdogoc + +my.trangbi[k].chiso.tocdogoc/100*+my.trangbi[k].tile + +my.trangbi[k].chiso.nangcap *+my.trangbi[k].tile;
}
...
function sd(id) {
 ...
 mypokemon(my.users.pokemon).kinang[k].ten = kinang(mypokemon(my.users.pokemon).kinang[k].id).ten;
				 mypokemon(my.users.pokemon).kinang[k].mota = kinang(+mypokemon(my.users.pokemon).kinang[k].id).mota;
  ...
  max = Math.floor(max);
   ...
   if(userID >=1) {
      ...
      if(+my.users.pokemon>=1) {
          ...
          if(+my.chiso.exp>= +my.chiso.expmax && +my.chiso.level <65) {
              my.chiso.exp = my.chiso.exp - level_my[+my.chiso.level];
              my.chiso.expmax = level_my[+my.chiso.level+1];
              my.chiso.hpgoc = +my.chiso.hpgoc+10;
              my.chiso.level = +my.chiso.level+1;
              my.chiso.giapgoc = +my.chiso.giapgoc+10;
              my.chiso.tocdogoc = +my.chiso.tocdogoc+10;
              my.chiso.vatligoc = +my.chiso.vatligoc+5;
              my.chiso.phepgoc = +my.chiso.phepgoc+10;
              my.chiso.mpgoc = +my.chiso.mpgoc+15;
              my.chiso.chimanggoc = +my.chiso.chimanggoc +0.2;
              my.chiso.xuyengiapgoc = +my.chiso.xuyengiapgoc +0.5;
          ...
          var theluc = {hp :2*+my.chiso.theluc,mp :3*+my.chiso.theluc,giap :0,tocdo :0,vatli :0,chimang :0,phep :0,xuyengiap :0};
          var sucmanh = {hp :0,mp :0,giap :0,tocdo :0,vatli :1*+my.chiso.sucmanh,chimang :0.01*+my.chiso.sucmanh,phep :0,xuyengiap :0};
          var tritue = {hp :4*+my.chiso.tritue,mp :0,giap :0,tocdo :0,vatli :0,chimang :0,phep :0,xuyengiap :0.01*+my.chiso.tritue};
           var kheoleo = {hp :0,mp :3*+my.chiso.kheoleo,giap :0,tocdo :3*+my.chiso.kheoleo,vatli :0,chimang :0,phep :0,xuyengiap :0};
            var thanphap = {hp :0,mp :0,giap :2*+my.chiso.thanphap,tocdo :0,vatli :0,chimang :0,phep :2*+my.chiso.thanphap,xuyengiap :0};
...
bum();
   ...
   if(rand(1,100) <= my.chiso.xuyengiap) {
    ...
    if(sucdanh.buff != null && +sucdanh.id !=8) {
        ...
        socket.emit("buff",{id : npcid.id  ,type : 'pokemon', buff : sucdanh.buff , uid : userID , time : sucdanh.timebuff , tacdung : sucdanh.tacdung });
   
        }
        
    }
    my.chiso.mp = my.chiso.mp - sucdanh.mp;
sucdanh.time = date()+ +sucdanh.hoichieu;

 if(+sucdanh.id ==8) {
socket.emit("buff",{id : userID  ,type : 'users', buff : sucdanh.buff , uid : userID , time : sucdanh.timebuff , tacdung : sucdanh.tacdung });
...
//	socket.emit("hp",{id : npcid.id , hp : dame });		
//	socket.emit("giet",{id : npcid.id});
...
function attpkm(id) {
   ...
   /////////////////POKEMON ATTACK///////////////////  
   var dame_quai = rand(npcid.tancong,npcid.tancong*3);
...
my.users.mapname = mapCode;
 ...
 $.nghia({
	url : code(),
	type : 'POST',
	data : {a : 'users' , uid : my.users.taikhoan , users : JSON.stringify(my)  },
...
function pvp(id) {
    ...
    if( +sucdanh.id ==58) {
          ...
          my.chiso.mp = my.chiso.mp - sucdanh.mp;
my.chiso.hp = my.chiso.hpfull;
        socket.emit("buff",{id : id ,type : 'users', buff : sucdanh.buff , uid : userID , time : sucdanh.timebuff , tacdung : sucdanh.tacdung });
...
socket.emit("buff",{id : userID  ,type : 'users', buff : sucdanh.buff , uid : userID , time : sucdanh.timebuff , tacdung : sucdanh.tacdung });
   ...
   if(rand(1,100) <= my.chiso.xuyengiap) {
  	...
  	att_msg+=' XUYÊN GIÁP';
       ...
       dame = dame - getusers(id).chiso.giap;
   } 
    ...
    /////////SEND DATA TO SOCKET.IO/////
  bum();
    socket.emit("damepvp",{id : id , dame : dame , uid : userID  });		
...
max : +hu_my.sohinh,
	  ...
	  socket.emit("kinang",{
		...
		thoigian : hu_my.sohinh,
		tinh : (hu_my.rong/hu_my.sohinh),
		canvas_x : (getuser(id).drawPosX)-20,
		canvas_y : (getuser(id).drawPosY)-10
			}); 
...
socket.emit("vatpham",{uid : userID ,  idvp : idvp , soluong : soluong , id : rand(1000000,999999999) , map : mapID , x : userEvent.mapPosition.X+1 , y : userEvent.mapPosition.Y+1});		
}

function nhatvp(id) {
socket.emit("nhatvatpham",{uid : userID , id : id });		
...
userEvent.mapPosition.Y=+getnpc(my.users.muctieu).mapPosition.Y;
userEvent.mapPosition.X=+getnpc(my.users.muctieu).mapPosition.X;
}
if(getnpc(my.users.muctieu) !== null) {
    my.click.id =  my.users.muctieu;

    if(userEvent.mapPosition.Y != getnpc(my.users.muctieu).mapPosition.Y) {
       userEvent.mapPosition.Y=+getnpc(my.users.muctieu).mapPosition.Y;
       dctt();
 
    }
    if(userEvent.mapPosition.X != getnpc(my.users.muctieu).mapPosition.X) {
               dctt();
