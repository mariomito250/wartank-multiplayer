...
// && AND
 ...
 var imgload = '/sql/load/377346.jpg';
var imglogo = '/sql/load/PokeRPG.png';
var version = '0.0.1';
(function($) {
...
load_tat();
 ...
 if(phantram>=100) {
     var nghia = 'Loading...';
		...
		ctx.font = "bold 12px sans-serif";
			ctx.textAlign = "center";
			drawShadowText(ctx,"Version "+version+" Beta", cvsWidth-70, cvsHeight-20);		
  		...
  		if(+my.chiso.hp >0 && +my.chiso.mp < +my.chiso.mpfull) {
  			  	    ...
  			  	    if(+my.chiso.mp >= +my.chiso.mpfull) {
  			  	        my.chiso.mp = +my.chiso.mpfull;
  			  	  			  	...
  			  	  			  	if(+my.users.thucan.load >=40) {
my.users.thucan.load=0;
}
  			  	
  			  	++my.users.thucan.load;	
  			  			
  			    }
}

if(my.users.autobat ==1) {
      			  			drawShadowText(ctx,"Auto On , TG Nhặt : "+thoigiantinh(my.users.autotime)+" ",cvsWidth-27, 95,"#00FF33");
...
++my.users.tauto;
  	...
  	drawShadowText(ctx,"lv."+getnpc(my.click.id).level+" "+getnpc(my.click.id).name+" "+tron(getnpc(my.click.id).hp)+"/"+tron(getnpc(my.click.id).hpfull)+"",cvsWidth-20, 281);	
  }
 if(my.click.type =="pokemon" && getnpc(my.click.id) === null) { 
...
if(sd(updatekinang(1).id).time > date()) {
ctx.fillStyle = "#585858";
ctx.fillRect(15,80,40, ( 40* (+sd(updatekinang(1).id).time - date()) / +sd(updatekinang(1).id).hoichieu   )   );
...
if(sd(updatekinang(2).id).time > date()) {
ctx.fillStyle = "#585858";
ctx.fillRect(55,80,40, ( 40* (+sd(updatekinang(2).id).time - date()) / +sd(updatekinang(2).id).hoichieu   )   );
...
if(sd(updatekinang(3).id).time > date()) {
ctx.fillStyle = "#585858";
ctx.fillRect(95,80,40, (40*(+sd(updatekinang(3).id).time - date())/+sd(updatekinang(3).id).hoichieu));
...
if(sd(updatekinang(4).id).time > date()) {
ctx.fillStyle = "#585858";
ctx.fillRect(135,80,40, (40*(+sd(updatekinang(4).id).time - date())/+sd(updatekinang(4).id).hoichieu));
...
if(sd(updatekinang(5).id).time > date()) {
ctx.fillStyle = "#585858";
ctx.fillRect(175,80,40, (40*(+sd(updatekinang(5).id).time - date())/+sd(updatekinang(5).id).hoichieu));
...
url : code(),
type : 'POST',
data : {getmap :1,a:'map' , b : 'admin', mapid :mapCode},
ducnghia : function(result){
    loadMapEventsCallback(result);
}
});
...
if(userID == this.buff[k].uid && +getusers(this.id).chiso.hp >0) {
    this.truhp = +this.hp/100*+this.buff[k].tacdung;
this.hp = +this.hp - +this.truhp;
this.buff[k].timetacdung = date() + 1000;
	socket.emit("damepvp",{id : this.id , dame : this.truhp , uid : userID  });
    }
}
...
if(imgpkm(''+this.trainerimg+'') !=null) {
         	addCharset('/sql/pokemon/'+this.trainerimg+''); 
		...
		if( this.quan != null && caidat.play <=0) {
			ctx.drawImage(this.quan, this.drawStartX, this.drawStartY, 32, 32,  this.drawPosX-5, this.drawPosY, 32, 32 );
	...
	for(var k=1;k<dataRaw.length;k++) {
		...
		if( dataRaw[k] != "" ) {
		 ...
		 if(getuser(dataUser[0]).trainerimg != dataUser[2] ) {
			...
			user.hp =  $.parseJSON(dataUser[14]).hp;
			user.co = dataUser[15];
...
if(imgpkm(''+this.spriteName+'') !=null) {
         	addCharset('/sql/pokemon/'+this.spriteName+''); 
...
if(userID == this.buff[k].uid) {
    ...
    this.hp = +this.hp - +this.truhp;
this.buff[k].timetacdung = date() + 1000;
    socket.emit("dame",{id : this.id , dame : this.truhp   });
    }
}
	...
	if( activeScript.length > 0 ) {
		...
		if( scriptFunc == "server side" ) {
			...
			for(var i=0;i<items.length;i++) {
				...
				screenResources.push(new ResourceImage("sql/load/images/items/" + item[3],"item." + item[1] ));
			}
			...
			var args = scriptData.split("^");
			screenResources.push(new ResourceImage("sql/load/images/items/" + args[1],"item." + args[0] ));
...
url : code(),
type : 'POST',
data : {a:'map' , b : 'admin' , token : args , mapName : mapName },
ducnghia : function(result){
    loadServerSideCallback(result);
}
});
}
...
giaotiep('loading ....');
$.nghia({
url : '/data/login.php?login',
type : 'POST',
data : { taikhoan : taikhoan , matkhau : matkhau , dangnhap:1,sv : server,a:'login' },
ducnghia : function(data){
 if(data ==1) {
     giaotiep(t('không tìm thấy tài khoản ! Tài khoản không được viết hoa'));
 } else
 if(data ==2) {
     giaotiep(t('Mật khẩu không chính xác vui lòng kiểm tra lại.'));
 } else {
...
c(maychu.notice);
    ...
    my.attack ={};
    my.danhlan = [];
	  socket.emit("kick",data.users.id);
    c_giaotiep();
c_menu();
    c_vemap();
    soundEnabled=1;
   userName=data.users.username; 
      musicEnabled = data.users.amthanh;
userID =data.users.id;
clan_icon=0;
clan_viettat=0;
 socket.emit("ducnghialogin", "/ducnghia^"+data.users.id+"");
 mapName = data.users.mapname;
 map(data.users.mapname,+data.users.x-1+1,+data.users.y-1+1,data.users.mapname);
userSprite = data.users.skin;
 userEvent = new gameEvent();
		userEvent.initAsPlayer(Point(+data.users.x-1+1,+data.users.y-1+1));
		loadimggame();
...
function reg_choingay() {
   	var taikhoan = $('#taikhoan000').val();
    var matkhau = $('#matkhau000').val();

$.nghia({
url : '/data/login.php?regchoi',
type : 'POST',
  ...
  var khach ='';
   ...
   if(vietsub.length <=0) {
     ...
     if(getdata('taikhoan') !="null") {
        khach+='<div class="nutplay" onclick="dangnhap();">'+vi('CHƠI')+'.'+getdata('taikhoan')+'</div>';
    }
            khach+='<div class="nutplay" onclick="ngonngu()">LANGUAGE</div>';
            khach+='<div class="nutplay" onclick="me_ackhac()">'+vi('ĐỔI TÀI KHOẢN')+'</div>';
            khach+='<div class="nutplay" onclick="newac()">'+vi('ĐĂNG KÍ')+'</div>';
  var left= (nx/2)-80;
    var width = nx/2;
    var height = 'auto';
    var top = ny-250;
